...
@app.cell(hide_code=True)
def y_schaal(er_curve, ir_curve, np):
    _t=np.linspace(6.0,23.0,800)
    def _som(specs): return sum(fn(_t,mg,7.0+dh) for fn,mg,dh in specs)
    Y_MAX=float(max(
        _som([(ir_curve,10,0),(ir_curve,10,4),(ir_curve,5,8)]).max(),
        _som([(er_curve,27,0)]).max(),
        _som([(er_curve,27,0),(ir_curve,10,6)]).max(),
        _som([(ir_curve,10,0),(er_curve,27,0.5)]).max(),
    ))*1.18
    return (Y_MAX,)
...
def grafiek(
    ...
    if active=="Schema 1 \u2014 Alleen IR":
        doses=[
            {"mg":10,"fn":ir_curve,"t0":wake,           "label":"10 mg IR",      "color":"#f5834a"},
            {"mg":10,"fn":ir_curve,"t0":wake+s1_d2.value,"label":"10 mg IR",     "color":"#f5834a"},
            {"mg": 5,"fn":ir_curve,"t0":wake+s1_d3.value,"label":"5 mg IR (half)","color":"#b48afa"},
        ]
    elif active=="Schema 2 \u2014 Alleen ER":
        doses=[{"mg":27,"fn":er_curve,"t0":wake,"label":"27 mg Concerta ER","color":"#5b9cf6"}]
    elif active=="Schema 3 \u2014 ER + IR":
        doses=[
            {"mg":27,"fn":er_curve,"t0":wake,           "label":"27 mg Concerta ER","color":"#5b9cf6"},
            {"mg":10,"fn":ir_curve,"t0":wake+s3_ir.value,"label":"10 mg IR",        "color":"#f5834a"},
        ]
    else:
        doses=[
            {"mg":10,"fn":ir_curve,"t0":wake,           "label":"10 mg IR",          "color":"#f5834a"},
            {"mg":27,"fn":er_curve,"t0":wake+s4_er.value,"label":"27 mg Concerta ER","color":"#5b9cf6"},
        ]
    ...
    df_total=pd.DataFrame([{"t":float(ti),"Tijd":fmt(float(ti)),"ng_mL":round(float(ci),4)}
    ...
    y=alt.Y("ng_mL:Q",scale=alt.Scale(domain=[0,Y_MAX]),
            axis=alt.Axis(title="ng/mL (gepersonaliseerd)",grid=True,gridColor="#e8e8e8",labelColor="#555"))
    y_tot=alt.Y("ng_mL:Q",scale=alt.Scale(domain=[0,Y_MAX]))
    col=alt.Color("Dosis:N",scale=alt.Scale(domain=labels,range=colors),
    ...
    lines=alt.Chart(df).mark_line(strokeWidth=2.2,interpolate="monotone").encode(
        x=x,y=y,color=alt.Color("Dosis:N",scale=alt.Scale(domain=labels,range=colors),legend=None))
    total_line=alt.Chart(df_total).mark_line(strokeWidth=2.8,strokeDash=[7,3],
        interpolate="monotone",color="#2db37a").encode(x=x,y=y_tot)

    rb_area=(alt.Chart(df_rb_zone).mark_area(opacity=0.18,color="#e05555",interpolate="monotone")
    ...
    thr_line=alt.Chart(df_thr).mark_line(strokeDash=[4,4],strokeWidth=1.5,
        color="#e05555",opacity=0.7).encode(x=alt.X("t:Q"),y=alt.Y("ng_mL:Q",scale=alt.Scale(domain=[0,Y_MAX])))
    ...
    if rebound_t is not None:
        rb_layers.append(
            alt.Chart(pd.DataFrame([{"t":rebound_t,"label":f"Rebound ~{fmt(rebound_t)}"}]))
            .mark_rule(color="#e05555",strokeWidth=2,opacity=0.8)
            .encode(x=alt.X("t:Q"),tooltip=alt.Tooltip("label:N")))
    ...
    dot=(alt.Chart(df_total).mark_point(filled=True,size=70,color="#2db37a")
